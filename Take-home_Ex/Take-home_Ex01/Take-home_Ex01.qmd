...
```{r}
pacman::p_load(tidyverse, lubridate, stringr, ggplot2, knitr, dplyr, gridExtra, plotly, gganimate,transformr, cluster, GGally)
```
...
1.  X-axis (Timestamp): Represents the time variable, showing the progression of the data over time.

2.  Y-axis (Moving Average Joviality): Represents the moving average of joviality calculated for each education level at each timestamp. The moving average smooths out the short-term fluctuations in joviality and highlights the overall trend.

3.  Color-coded lines (Education Level): Each line in the plot represents a different education level. The color-coding allows you to easily distinguish between the education levels and observe their trends in joviality over time.
